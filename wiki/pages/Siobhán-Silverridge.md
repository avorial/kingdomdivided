---
category: character
type: character
name: Siobhán Silverridge
summary: ""
tags:
  - misc
visibility: players
approvalStatus: approved
knownToPlayers: true
keyLinks: []
aliases: []
lastEditedBy: patricthomas
parent: Belllads-Chronicle-Player-Characters
---

![shivon_silverridge](/wiki/media/shivon_silverridge.jpg)

*Siobhán Silverridge*
Shivon Silverridge was imported from the Kingdom Divided 1 Sword Chronicle Foundry actor data.

<!-- sword-chronicle-sheet:start -->
## Sword Chronicle Sheet

```sword-chronicle-sheet
name: "Siobhán Silverridge"
gender: "Female"
portrait: "shivon_silverridge.jpg"
destiny: 5
destinySpent: 3
abilities:
  "Admiralty": 2
  "Agility": 2
  "Animal Handling": 2
  "Athletics": 2
  "Awareness": 3
  "Cunning": 4
  "Deception": 3
  "Endurance": 2
  "Fighting": 2
  "Healing": 2
  "Knowledge": 3
  "Language": 3
  "Marksmanship": 1
  "Nautical": 2
  "Persuasion": 4
  "Status": 6
  "Stealth": 3
  "Survival": 2
  "Thievery": 3
  "Warcraft": 2
  "Will": 3
armor:
  name: "Noble's Garb (Excellent)"
  rating: 0
  penalty: 0
attacks:
  - name: "Crossbow, Light"
  - name: "Stiletto"
damage: 2
qualities: ["Ferdor"]
benefits:
  - "Attractive"
  - "Charismatic"
  - "Favored"
drawbacks:
  - "Fear"
equipment:
  - "\"Adair\" Coursers (War Horse)"
  - "Mysteries of the Ruins of Windbreak (Book)"
  - "Thick Dress 1 (spare armour)"
appearance:
  features: "Copper Red Hair, Pretty"
history: "Spends: 1 Reputation (Reputation 1B) and 1 Empathy) (Empathy 2B) - 20 XP Spend : 1 Status (XP Spend 2) - 30 XP 1 Status (XP Spend 2 - Jun 27, 2023) - 30 XP"
notes: "CurrentDisposition: 7"
```

<details>
<summary>Foundry import data</summary>

Imported from Kingdom Divided 1 Foundry actor data on 2026-07-10.
### Foundry Notes

#### Personal History

Spends: 1 Reputation (Reputation 1B) and 1 Empathy) (Empathy 2B) - 20 XP

Spend : 1 Status (XP Spend 2) - 30 XP

1 Status (XP Spend 2 - Jun 27, 2023) - 30 XP

#### Allies

Amund Henge (Amiable)

Viscount Somchai Frick (Almirn) (Friendly)

@UUID[Actor.ALDaZWm8V5Mg6egw]{Anya Mosley}

#### Distinguishing Features

Copper Red Hair, Pretty

#### Ancestry Notes

Ferdor

### Character Data

| Field | Value |
| --- | --- |
| Gender | Female |
| DestinyPoints Current | 2 |
| DestinyPoints Max | 5 |
| SorceryPoints Current | 0 |
| SorceryPoints Max | 0 |
| DerivedStats IntrigueDefense | 13 |
| DerivedStats Composure Value 0 | 9 |
| DerivedStats Composure Value 1 | 9 |
| DerivedStats Composure Modifier | 0 |
| DerivedStats Composure Current | 0 |
| DerivedStats Frustration | 0 |
| DerivedStats CombatDefense Value 0 | 7 |
| DerivedStats CombatDefense Value 1 | 7 |
| DerivedStats CombatDefense Modifier | 0 |
| DerivedStats Health Value 0 | 6 |
| DerivedStats Health Value 1 | 6 |
| DerivedStats Health Modifier | 0 |
| DerivedStats Health Current | 2 |
| DerivedStats Fatigue | 0 |
| DerivedStats ArmorRating | 0 |
| DerivedStats Movement RunBonus | 0 |
| DerivedStats Movement Total | 3 |
| CurrentDisposition | 7 |
| Modifiers Bulk 0 Mod | 1 |
| Modifiers Bulk 0 IsDocument | true |
| Modifiers Bulk 1 Mod | 1 |
| Modifiers Bulk 1 IsDocument | true |
| Modifiers Agility 0 Mod | 0 |
| Modifiers Agility 0 IsDocument | true |

### Foundry Items

#### Ability

- Admiralty 2
- Agility 2
- Animal Handling 2
- Athletics 2
- Awareness 3
- Cunning 4
- Deception 3
- Endurance 2
- Fighting 2
- Healing 2
- Knowledge 3
- Language 3
- Marksmanship 1
- Nautical 2
- Persuasion 4
- Status 6
- Stealth 3
- Survival 2
- Thievery 3
- Warcraft 2
- Will 3

#### Armor

- Noble's Garb (Excellent) 0
- Thick Dress 1

#### Benefit

- Attractive
- Charismatic
- Favored

#### Drawback

- Fear

#### Equipment

- "Adair" Coursers (War Horse)
- Mysteries of the Ruins of Windbreak (Book)

#### Weapon

- Crossbow, Light
- Stiletto

</details>
<!-- sword-chronicle-sheet:end -->
